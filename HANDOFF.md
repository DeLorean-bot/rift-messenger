...
4. Реальная runtime-проверка автообновления. Криптографическая цепочка проверена (см. статус ниже), но полноценный тест «поставил старую версию → запустил → приложение само обновилось» ещё не проводился. С 0.4.0 его сделать нельзя (там updater не существовал). Первый честный тест возможен только 0.5.0 → 0.5.1: собери и опубликуй 0.5.1, установи 0.5.0 и убедись, что mandatory updater её подтягивает.
5. Старый installer 0.4.0 нельзя удалённо заблокировать: в нём updater ещё не существовал. Обязательное обновление начинает работать только с 0.5.0.
...
- НЕ сделано: реальный runtime-тест автообновления на живом Windows (нужна 0.5.1 или ручная установка; 0.4.0 обновляться не умеет).
