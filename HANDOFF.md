...
1. Проверить RNNoise + новый mic-пайплайн на настоящем микрофоне в Tauri Windows-приложении: выбор устройства, gain, push-to-talk, toggle RNNoise, mute, hangup, повторный вход. Особое внимание — чтобы raw mic и processed track всегда останавливались.
2. Active-speaker detection (WebAudio analyser) — точный индикатор «говорит» и presence для PTT. P2P-совместимо.
3. Adaptive bitrate для 1:1 (RTP stats + `setParameters`). P2P-совместимо.
