...
- Текстовый столп уровня Discord (2026-07-23): typing indicator («Собеседник печатает…», троттлинг 3с), статусы доставки (sent ✓ / delivered ✓✓ / read ✓✓ акцентом; read-акки флашатся при фокусе канала), reply (бар в композере + цитата над сообщением), edit («(изм.)»), delete (у обеих сторон), emoji-реакции (чипы с счётчиком, toggle с обеих сторон), hover-меню действий на сообщении. Всё по data channel (`typing`, `msg-ack`, `message-edit`, `message-delete`, `reaction`); поля Message расширены в `storage.ts`. Проверено E2E двумя клиентами: typing + доставка + реакция.
- Внешний вид (идея из темы Discord11): акцентный цвет через `--accent-h` (HSL) — пресеты + слайдер оттенка в модалке профиля, все жёсткие rgba-подсветки переведены на `color-mix(... var(--acid))`; стиль «сообщения-пузыри» (Win11). Хранится в localStorage. Тема Discord11 напрямую НЕ применима (стилизует хэш-классы Discord), взяты только идеи.
- Screen share доведён и **подтверждён владельцем на живом использовании** (2026-07-23): fullscreen плиток (dblclick + hover-кнопка), превью демки переживает сворачивание/разворачивание звонка (attachCallStreams предпочитает screen stream), скрин-аудио на выделенном sendonly-трансивере (не хайджекает микрофон, реюзается между сессиями, ref сбрасывается в createPeer), смена качества на лету свапает и аудио-трек и глушит все старые треки (утечка захвата устранена).
